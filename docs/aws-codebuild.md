...
- `ECR_REPOSITORY`: ECR repository name, for example `ticketmaster/booking-service`.

## Optional variables

- `AWS_ACCOUNT_ID`: If omitted, CodeBuild resolves it with STS.
- `IMAGE_TAG`: If omitted, the build uses `booking-service-<short-commit-sha>`.
- `CONTAINER_NAME`: ECS task definition container name. Defaults to `booking-service`.
